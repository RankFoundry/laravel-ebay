<?php
namespace Rankfoundry\eBaySDK\Types;

use \Rankfoundry\eBaySDK\Types;
use \Rankfoundry\eBaySDK\Exceptions;
use \Rankfoundry\eBaySDK\JmesPath\Env;
use \Rankfoundry\eBaySDK\JmesPath\JmesPathableObjectInterface;
...
class BaseType implements JmesPathableObjectInterface
{
     ...
     *
     * @throws \Rankfoundry\eBaySDK\Exceptions\UnknownPropertyException If the property does not exist.
     * @throws \Rankfoundry\eBaySDK\Exceptions\InvalidPropertyTypeException If the value is the wrong type for the property.
     */
     ...
     * @param mixed $value. The value to assign to the property.
     *
     * @throws \Rankfoundry\eBaySDK\Exceptions\InvalidPropertyTypeException If trying to assign a non array type to an repeatable property.
     */
    private function setValue($class, $name, $value)
    {
        ...
        if (!$info['repeatable']) {
            ...
            if ('array' !== $actualType) {
                throw new Exceptions\InvalidPropertyTypeException($name, 'Rankfoundry\eBaySDK\Types\RepeatableType', $actualType);
            } else {
    ...
    private static function ensurePropertyType($class, $name, $value)
    {
        ...
        foreach ($valid as $check) {
            if ($check !== 'any' && \Rankfoundry\eBaySDK\checkPropertyType($check)) {
    ...
    {
        if (is_subclass_of($value, '\Rankfoundry\eBaySDK\Types\BaseType', false)) {
